...
> 2026-07-19 活跃市值择时覆盖层与四赛道对照（优先阅读本节）

## 本次完成

- 已将用户手工给出的指南针 0AMV 活跃市值日期实现为可选市场状态覆盖层，不修改 B1 公式、候选排序、费用、T+1、最多三仓和分批退出规则。
- 状态从 2026-06-01 的 `risk_off` 开始；单日 +4% 定义在 6 月 15 日转为 `risk_on`，7 月 2 日转回 `risk_off`。两日累计 +4% 定义额外保留 6 月 22 日确认事件，但不会重置持仓、峰值、持仓日或到期日；两种定义的有效状态窗口逐交易日完全一致。
- 主模式 `same_day_1455` 在事件日 14:55 生效：上涨日允许 14:55 入场，下跌日先提交 `market_regime_exit` 再处理普通止损/止盈/到期，且禁止当日新仓。保守控制 `next_session_0935` 在下一交易日 09:35 生效；事件日 14:55 仍使用旧状态。
- 强制清仓沿用 T+1、跌停锁定、卖出滑点、佣金、印花税、100 股整手和单根五分钟成交量 10% 上限；部分成交持续到下一可成交 bar。即使个股缺少事件时刻 bar，清仓意图也先持久化，不会丢失或虚构成交。
- 已增加条件产物 `market_regime.csv`、`market_regime_exit` 成交原因、`market_regime_off` 拒单原因和配置路径/SHA-256。未传 `--market-regime` 时，原 CLI 文件集、manifest 字段和报告文本保持不变。
- 7 月 17 日的 next-session 事件在样本外 7 月 20 日 09:35 生效。两份 next-session 配置显式记录该交易日扩展和上交所 2026 年休市安排来源；扩展仅保留审计时间线，不加载未来价格、不生成成交，也不延长收益区间。

## 真实矩阵与审计

- 分析期固定为 2026-06-01 至 2026-07-17，使用四个冻结扫描、两份 qfq 快照、四个股票池和两份仓位配置。
- 已完成 `4 个股票池 x 2 个单票目标仓位 x 2 个生效时点 = 16` 个真实 timed run。两种活跃市值定义先证明状态窗口相同，再只执行一次组合回测，避免重复计算同一账户路径。
- 16 个 run 均在提交 `8f1c3c2`、`git_dirty=false` 下重新生成；每组标准产物和 `market_regime.csv` 齐全，驱动器退出码为 0。落盘后再次独立执行 16/16 artifact reconciliation，全部通过。
- reconciliation 覆盖：最终现金 + 市值 = NAV、买入股数 = 卖出股数 + 最终余量、最多三仓、同股持仓不重叠、`risk_off` 或清仓余单期间无新仓、跨整日缺失分钟 bar 的状态转换、完整执行参数、全部交易汇总字段，以及分钟/qfq/扫描/股票池/配置/运行清单哈希与 verified baseline 一致。
- 完整 ledgers 位于本地 ignored 目录 `outputs/intraday_b1_active_market_cap_20260719/`；轻量对照位于 `outputs/intraday_b1_active_market_cap_20260719_summary/`，包含 24 行 baseline/same-day/next-session 对照、34 行事件审计、来源哈希和研究报告。

## 结果摘要

| 股票池 | 单票目标 | untimed 收益 / 5m 回撤 | same-day 收益 / 5m 回撤 / 平仓胜率 | next-session 收益 / 5m 回撤 / 平仓胜率 |
| --- | ---: | ---: | ---: | ---: |
| AI/半导体 | 1/3 | -0.5570% / -18.8312% | 8.6573% / -6.0444% / 6 笔、66.67% | 2.3338% / -7.3498% / 5 笔、60.00% |
| AI/半导体 | 1/4 | 0.7195% / -13.7857% | 6.5548% / -4.5460% / 6 笔、66.67% | 1.9461% / -5.4722% / 5 笔、60.00% |
| 创新药 30 | 1/3 | 9.9406% / -4.5422% | 0.0000% / 0.0000% / 0 笔 | 0.0000% / 0.0000% / 0 笔 |
| 创新药 30 | 1/4 | 7.4639% / -4.0890% | 0.0000% / 0.0000% / 0 笔 | 0.0000% / 0.0000% / 0 笔 |
| 中证电池 | 1/3 | -12.9414% / -16.0113% | -0.4419% / -5.2375% / 3 笔、66.67% | -0.0177% / -5.2375% / 3 笔、66.67% |
| 中证电池 | 1/4 | -9.8990% / -12.1672% | -0.3680% / -3.9938% / 3 笔、66.67% | -0.0480% / -3.9938% / 3 笔、66.67% |
| 国证机器人产业代理 | 1/3 | 9.3685% / -12.2537% | 2.7378% / -4.6021% / 3 笔、33.33% | 0.1614% / -4.5892% / 2 笔、50.00% |
| 国证机器人产业代理 | 1/4 | 7.8055% / -9.4804% | 2.0265% / -3.3719% / 3 笔、33.33% | 0.1158% / -3.4159% / 2 笔、50.00% |

- AI same-day 相对 next-session 的收益优势为 6.32/4.61 个百分点，但 same-day 假设完整日活跃市值信号在 14:55 已可见，存在潜在前视；更保守的 next-session 仍为正收益，但优势明显缩小。
- 12 个有实际暴露的 timed 行中，8 行收益高于 baseline；三个有暴露股票池中，AI 和电池在所有仓位/时点下改善收益，机器人全部落后 baseline，因此收益改善不跨股票池一致。
- 12/12 个有暴露 timed 行的五分钟、盘中低价和日线回撤均优于 baseline；但资金利用率和交易数显著下降，回撤改善很大部分来自减少市场暴露，不能自动归因于更好的选股。
- 创新药四个 timed 行均为 0 笔交易、0 利用率；0 收益和 0 回撤表示没有暴露，不是策略改善。
- timed 非零样本每行只有 2-6 笔平仓；审计规则统一把 1-6 笔都标为薄样本，胜率和利润因子不稳定。本轮日期和 4%/-3% 阈值均为手工、事后输入，没有做阈值或 B1 参数优化，不能据此宣称已验证择时 alpha。

## 代码与验证

- 设计：`docs/superpowers/specs/2026-07-19-active-market-cap-timing-design.md`；实施计划：`docs/superpowers/plans/2026-07-19-active-market-cap-timing.md`。
- 新增状态模块、组合/CLI 覆盖层、16 组矩阵驱动器、artifact reconciliation 和可复现紧凑汇总器；未引入网络数据依赖，日历来源只作为配置审计元数据。
- 当前完整测试为 176/176 通过；真实摘要逐项从 NAV/fills/trades/positions 重算，24 个 key 唯一，224 个来源哈希和 3 个生成文件哈希全部匹配。
- 独立任务级审查已修复事件时间戳覆盖、整日缺 bar 丢失状态转换、普通退出冒充状态退出、CLI 来源字节漂移、旧目录残留、矩阵完成清单误认证、完整执行参数与交易经济性未重算，以及报告单位/硬编码结论等问题。

## 未完成与下一步

1. 获取逐日原始 0AMV 数值和 14:55 前可观测快照，替代手工事件日期，验证 same-day 是否真实可执行。
2. 在更长、独立时期预注册 4%/-3% 阈值并做 walk-forward；当前 2026-06/07 样本只能作为探索性归因。
3. 扩大 timed 平仓样本，重点复核机器人“少回撤但错失收益”和创新药“完全无暴露”是否持续。
4. 继续把动态历史成分、ST/停牌状态和盘口成交约束加入组合层，再评估是否进入模拟盘。
