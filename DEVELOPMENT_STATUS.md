...
- 16 个 run 均在提交 `316140a`、`git_dirty=false` 下生成；每组标准产物和 `market_regime.csv` 齐全，驱动器退出码为 0。落盘后再次独立执行 16/16 artifact reconciliation，全部通过。
- reconciliation 覆盖：最终现金 + 市值 = NAV、买入股数 = 卖出股数 + 最终余量、最多三仓、同股持仓不重叠、`risk_off` 或清仓余单期间无新仓、分钟/qfq/扫描/股票池/配置哈希与 verified baseline 一致。
...
- timed 非零样本每行只有 2-6 笔平仓，胜率和利润因子不稳定。本轮日期和 4%/-3% 阈值均为手工、事后输入，没有做阈值或 B1 参数优化，不能据此宣称已验证择时 alpha。
...
- 当前完整测试在 Task 6 完成时为 160/160 通过；真实摘要逐项从 NAV/fills/trades 重算，最大误差 `8.88e-16`，24 个 key 唯一，184 个来源哈希和 3 个生成文件哈希全部匹配。
- 独立任务级审查已修复事件时间戳覆盖、缺 bar 丢失清仓意图、CLI 来源字节漂移、旧目录残留、矩阵账本误认证和报告单位/硬编码结论等问题。
